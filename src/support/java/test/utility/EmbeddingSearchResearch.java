...
public class EmbeddingSearchResearch {
  ...
  private static final TextEmbedding[] hits = {TextEmbedding.fromResource("hitDoc1.txt"),
      TextEmbedding.fromResource("hitDoc2.txt"), TextEmbedding.fromResource("hitDoc3.txt"),};
  private static final TextEmbedding[] misses = {TextEmbedding.fromResource("missDoc1.txt"),};
  private static final TextEmbedding[] queries = {TextEmbedding.fromResource("query1.txt"),
      TextEmbedding.fromResource("query2.txt"), TextEmbedding.fromResource("query3.txt"),};
  ...
  public void _02_cosine() throws Exception {
    ...
    for (final TextEmbedding query : queries) {
      ...
      for (final Entry<Double, String> entry : map.entrySet()) {
        System.out.printf("{\"score\"=\"%f\", \"text\"=\"%s\"}%n", entry.getKey(),
            entry.getValue());
      }
    }
  }
  ...
  private org.apache.lucene.search.Query buildEmbeddingQuery(final TextEmbedding query)
      throws ParseException {
    System.out.printf("%n{\"query\"=\"%s\"}%n", query.text().text());
    return buildQuery(null, query.embedding());
  }

  private Query buildQuery(final String query, final Embedding embedding) {
    ...
    final boolean onlyMatches = true;
    if (!onlyMatches) {
